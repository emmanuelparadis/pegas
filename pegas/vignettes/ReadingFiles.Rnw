...
function \code{read.vcf}.

<<>>=
args(read.vcf)
@ 
Because VCF files can be very large, only 1000 loci are read. The option
\code{skip} specifies how many loci to skip before reading the data,
so that a file can be read and/or analysed in successive steps.

\code{read.vcf} returns an object of class \code{"loci"} with the
following additional attributes:

\begin{description}
\item[\texttt{CHR}:] the chromosome number (integer values);
\item[\texttt{POS}:] the position of the locus (numeric values);
\item[\texttt{QUAL}:] the quality of the inferred locus (integer values);
\item[\texttt{FILTER}:] the filter (characters).
\end{description}

VCF files contain a lot more information (genotype likelihoods, \dots)
which are (currently) not read by \code{read.vcf}.
