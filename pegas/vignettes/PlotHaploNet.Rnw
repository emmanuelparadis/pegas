...
(\code{?plot.haploNet} and \code{?mutations}) for full details. The
function \code{plotNetMDS}, which offers an alternative approach to
plot networks, is not considered in this document.
...
that they are plotted with the same \code{plot} method.
...
\code{fast = TRUE} is used when calling \code{plot}.
...
``backbone'' network based on an MST is used, so there are no
reticulations and the equal-angle algorithm makes sure that there are
no segment-crossings. In practice, it is likely that this backbone MST
is arbitrary with respect to the rest of the network. The other
segments are then drawn on ``top'' of this MST.
...
We illustrate the procedure with the `woodmouse' data, a set of
sequences of cytochrome \textit{b} from fifteen woodmice
...
sample, with replacement, eighty sequences, and create two grouping,
hierarchical variables: \code{region} with two levels each containing
forty haplotypes, and \code{pop} with four levels each containing
twenty haplotypes:
...
\noindent We extract the haplotypes before reconstructing the RMST
after computing the pairwise Hamming distances:
...
<<echo=false,quiet=true>>=
if (!exists("oldopts")) oldopts <- getHaploNetOptions()
do.call(setHaploNetOptions, oldopts)
@
...
\noindent We compare the layout after setting \code{fast = TRUE}:

<<fig=true>>=
plot(nt, fast = TRUE)
@

\noindent By default, not all links are drawn. This is controlled by
the option \code{threshold} which takes two values to set the lower
...
(\code{x}) is the object to be plotted. Until \pegas\ 0.14, all but
the first arguments were defined with default values. In recent
versions, as shown above, only \code{size} and \code{shape} are
defined; the other options, if not modified in the call to
\code{plot}, are taken from a set of parameters which can be modified
as explained in Section~\ref{sec:getsetopts}.

The motivation for this is, in most practical applications of this
function, the user will want to modify \code{size} and \code{shape}
with their own data such as the haplotype frequencies or else, and
they might be changed repeatedly, for instance with different data
subsets. On the other hand, the other options are more likely to be
used to change the visual aspect of the graph so it could be more
useful to change them once during a session as explained below.
...
A similar mechanism can be used to show variables such \code{region}
...
beforehand. \code{replot}---which, of course, cannot be used in this
...
\item The final coordinates, for instance saved with \code{xy <-
    ...
    xy)}. This makes possible to input coordinates calculated with
  another software.
\item In previous versions, the plot tended to ``drift'' when
...
discs for the same values given to \code{size}. The figure below shows
...
other links. The option \code{style} takes the value either \code{"table"}
(the default) or \code{"sequence"}. In the second case, the positions
...
so that the user will find which ones to modify:

<<>>=
names(getHaploNetOptions())
@

We see here an example with the command \code{plot(nt, size = 2)}
which is repeated after a call to \code{setHaploNetOptions}:
